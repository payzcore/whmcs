...
class PayzCoreApi
{
     ...
     *     @type string $chain             Blockchain network (TRC20, BEP20, ERC20, POLYGON, ARBITRUM)
    ...
    {
        $response = $this->request('POST', '/api/v1/payments', $params);
    ...
    public function getPayment($paymentId)
    {
        ...
        $response  = $this->request('GET', '/api/v1/payments/' . $paymentId);
    ...
    public function confirmPayment($paymentId, $txHash, $confirmPath = '')
    {
        if (empty($confirmPath)) {
            $paymentId   = preg_replace('/[^a-zA-Z0-9\-]/', '', $paymentId);
            $confirmPath = '/api/v1/payments/' . $paymentId . '/confirm';
        }
     ...
     */
    public static function verifyWebhookSignature($rawBody, $signature, $webhookSecret)
    {
    ...
    private function request($method, $path, array $body = null)
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_USERAGENT      => 'PayzCore-WHMCS/1.1.1',
        ]);
