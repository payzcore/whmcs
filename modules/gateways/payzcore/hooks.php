 ...
 * Uses mod_payzcore_mappings for reliable chain/token lookup instead of
...
add_hook('AdminInvoicesControlsOutput', 1, function ($vars) {
    ...
    // Determine chain from mod_payzcore_mappings (reliable, dedicated table)
    $chain = 'TRC20'; // default fallback
    $token = 'USDT';
    try {
    ...
    $explorerInfo = isset($explorers[$chain]) ? $explorers[$chain] : $explorers['TRC20'];
    ...
    return '<div class="alert alert-info" style="margin-top:10px;">'
         ...
         . 'Chain: ' . htmlspecialchars($chain) . ' &middot; '
