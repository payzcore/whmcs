...
{
    return [
        'DisplayName'                => 'Stablecoin Payment (USDT/USDC)',
        'APIVersion'                 => '1.1',
    ...
    return [
        ...
        'setupGuide' => [
            ...
            'Description'  => '<div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:6px;padding:12px 16px;margin:4px 0;font-size:13px;line-height:1.7;">'
                            ...
                            . '4. Set your <strong>Webhook URL</strong> in the PayzCore project to: <code>' . ($_SERVER['HTTP_HOST'] ?? 'yourdomain.com') . '/modules/gateways/callback/payzcore.php</code><br>'
        ...
        'textWarning' => [
            ...
            'Rows'         => '3',
            'Cols'         => '60',
            'Default'      => 'Send only {token} on {chain} to this address. Sending other tokens or using the wrong network may result in loss.',
            'Description'  => 'Warning text. Use {token} and {chain} as placeholders.',
        ],
        'textTestMode' => [
 ...
 * Shows cached chain info or a prompt to configure.
 *
 * @return string HTML
 */
function _payzcore_renderConfigStatus()
{
    ...
    if (!empty($cached['chains']) && is_array($cached['chains'])) {
        $chainNames = [];
        foreach ($cached['chains'] as $c) {
            $name = $c['name'] ?? $c['chain'];
            $tokens = isset($c['tokens']) ? implode(', ', $c['tokens']) : 'USDT';
            $chainNames[] = $name . ' (' . $tokens . ')';
        }
        $chainList = implode(', ', $chainNames);
        ...
        return '<div style="background:#f0fdf4;border:1px solid #86efac;border-radius:6px;padding:12px 16px;font-size:13px;line-height:1.6;">'
             ...
             . '<strong>Available chains:</strong> ' . htmlspecialchars($chainList, ENT_QUOTES, 'UTF-8') . '<br>'
             . '<strong>Last sync:</strong> ' . $syncTime . '<br><br>'
             . '<em style="color:#6b7280;">Chain list syncs automatically when you open this page. Available chains are determined by your wallet configuration at <a href="https://app.payzcore.com" target="_blank">app.payzcore.com</a>.</em>'
             . '</div>';
    }
    ...
    return '<div style="background:#fef3c7;border:1px solid #fcd34d;border-radius:6px;padding:12px 16px;font-size:13px;line-height:1.6;">'
         ...
         . 'Available chains are determined by your wallet configuration at <a href="https://app.payzcore.com" target="_blank">app.payzcore.com</a>.'
         . '</div>';
    } catch (\Exception $e) {
...
function payzcore_link($params)
{
    ...
    // Multi-chain: get enabled chains and default token from cached API config
    $enabledChains = _payzcore_getEnabledChains($params);
    ...
    // Customizable text fields (admin-editable for i18n) — built with placeholder token/chain
    // These will be finalized once chain/token are determined
    $warningRaw = $params['textWarning']
        ?: 'Send only {token} on {chain} to this address. Sending other tokens or using the wrong network may result in loss.';
    ...
    try {
        if ($existingPaymentId) {
            ...
            // Determine chain/token from the existing payment for text rendering
            $chain = $payment['chain'] ?? $enabledChains[0];
            $token = $payment['token'] ?? $defaultToken;
            $chainLabel = isset($chainLabels[$chain]) ? $chainLabels[$chain] : $chain;
            $texts = _payzcore_buildTexts($params, $token, $chainLabel, $warningRaw);
            ...
            // If expired or cancelled, create a new one (fall through to chain selection or creation)
        ...
        // --- Determine chain and token for new payment ---

        // Check if customer submitted the chain selector form
        $selectedChain = isset($_POST['payzcore_chain']) ? $_POST['payzcore_chain'] : null;
        $selectedToken = isset($_POST['payzcore_token']) ? $_POST['payzcore_token'] : null;

        // If multiple chains enabled and customer hasn't selected yet, show selector
        if (count($enabledChains) > 1 && $selectedChain === null) {
            return _payzcore_renderChainSelector($params, $enabledChains, $defaultToken, $invoiceId, $amount, $testMode);
        }

        // Resolve final chain and token
        if ($selectedChain !== null) {
            // Validate submitted chain is in the enabled list
            $chain = in_array($selectedChain, $enabledChains, true) ? $selectedChain : $enabledChains[0];
            $token = ($selectedToken === 'USDT' || $selectedToken === 'USDC') ? $selectedToken : $defaultToken;
        } else {
            // Single chain enabled, use it directly
        ...
        // Build texts with resolved chain/token
        $chainLabel = isset($chainLabels[$chain]) ? $chainLabels[$chain] : $chain;
        $texts = _payzcore_buildTexts($params, $token, $chainLabel, $warningRaw);
...
/**
 * Build the admin-customizable text array with resolved token and chain placeholders.
 *
 * @param array  $params     WHMCS gateway parameters
 * @param string $token      Resolved token (USDT/USDC)
 * @param string $chainLabel Human-readable chain label
 * @param string $warningRaw Raw warning text with {token}/{chain} placeholders
 * @return array
 */
function _payzcore_buildTexts($params, $token, $chainLabel, $warningRaw)
{
    return [
        ...
        'warning'          => str_replace(['{token}', '{chain}'], [$token, $chainLabel], $warningRaw),
        'test_mode'        => $params['textTestMode'] ?: 'TEST MODE - Monitoring is active but logging is verbose',
    ];
}

/**
 * Render the chain and token selector form for multi-chain checkout.
 *
 * Displayed when multiple chains are enabled and the customer has not yet
 * selected a network. The form posts back to the same invoice page.
 *
 * @param array  $params        WHMCS gateway parameters
 * @param array  $enabledChains List of enabled chain identifiers
 ...
 * @return string HTML output
 */
function _payzcore_renderChainSelector($params, $enabledChains, $defaultToken, $invoiceId, $amount, $testMode)
{
    $chainOptions = [
    ...
    // Build chain <option> tags
    $chainOptionsHtml = '';
    foreach ($enabledChains as $chain) {
        $info  = isset($chainOptions[$chain]) ? $chainOptions[$chain] : ['label' => $chain, 'desc' => ''];
        $label = htmlspecialchars($info['label'], ENT_QUOTES, 'UTF-8');
        $desc  = !empty($info['desc']) ? ' - ' . htmlspecialchars($info['desc'], ENT_QUOTES, 'UTF-8') : '';
        $val   = htmlspecialchars($chain, ENT_QUOTES, 'UTF-8');
        $chainOptionsHtml .= '<option value="' . $val . '">' . $label . $desc . '</option>';
    }
    ...
    // Determine if TRC20 is the first/default chain (to set initial token visibility)
    $firstChain      = $enabledChains[0];
    $tokenRowDisplay = ($firstChain === 'TRC20') ? 'none' : 'block';
    ...
    $html = <<<HTML
<div id="payzcore-chain-selector" style="
    max-width: 440px;
    margin: 20px auto;
    ...
    ">
        ...
        <form method="post">
            ...
            <div style="margin-bottom:16px;">
                ...
                <select name="payzcore_chain" id="payzcore-chain-select" style="{$selectStyle}">
                    {$chainOptionsHtml}
                </select>
            </div>
            ...
            <!-- Chain info hint -->
            <div id="payzcore-chain-hint" style="
...
<script>
(function() {
    var chainSelect = document.getElementById('payzcore-chain-select');
    ...
    var hintEl      = document.getElementById('payzcore-chain-hint');
    ...
    function updateUI() {
        ...
        // Update hint
        if (hintEl && chainHints[chain]) {
            hintEl.textContent = chainHints[chain];
    ...
    chainSelect.addEventListener('change', updateUI);
...
function payzcore_activate()
{
    ...
    // Add columns for existing installations (idempotent ALTER IGNORE)
    full_query("ALTER TABLE `mod_payzcore_mappings` ADD COLUMN IF NOT EXISTS `notice` text DEFAULT NULL");
    full_query("ALTER TABLE `mod_payzcore_mappings` ADD COLUMN IF NOT EXISTS `requires_txid` tinyint(1) NOT NULL DEFAULT 0");
    full_query("ALTER TABLE `mod_payzcore_mappings` ADD COLUMN IF NOT EXISTS `confirm_endpoint` varchar(500) DEFAULT NULL");
...
{
    full_query("DROP TABLE IF EXISTS `mod_payzcore_mappings`");

    return ['status' => 'success', 'description' => 'PayzCore gateway deactivated.'];
}

/**
 * Get the list of enabled blockchain chains from cached API config.
 *
 * Reads from cached /api/v1/config response. If no cache exists,
 * attempts a fresh API fetch. Falls back to TRC20 as a last resort.
 *
 * @param array $params WHMCS gateway parameters
 * @return array List of chain identifiers (e.g. ['TRC20', 'BEP20'])
 */
function _payzcore_getEnabledChains($params)
{
    // Try cached config first
    $cached = _payzcore_getCachedConfig();
    if (!empty($cached['chains'])) {
        $chains = [];
        foreach ($cached['chains'] as $c) {
            if (isset($c['chain'])) {
                $chains[] = $c['chain'];
...
{
    $url = rtrim($apiUrl, '/') . '/api/v1/config';
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_HTTPHEADER     => [
            ...
            'User-Agent: PayzCore-WHMCS/1.1.1',
        ],
 ...
 * @param array  $extra     Optional extra fields (amount, chain, token, address)
 */
function _payzcore_storePaymentId($invoiceId, $paymentId, $extra = [])
{
    ...
    $data = [
        ...
        'chain'            => isset($extra['chain']) ? $extra['chain'] : 'TRC20',
    ...
    full_query(
        "INSERT INTO `mod_payzcore_mappings` (`invoice_id`, `payment_id`, `amount`, `chain`, `token`, `address`, `status`, `notice`, `requires_txid`, `confirm_endpoint`)
         VALUES ('" . intval($data['invoice_id']) . "', '" . db_escape_string($data['payment_id']) . "',
                 '" . floatval($data['amount']) . "', '" . db_escape_string($data['chain']) . "',
...
function _payzcore_renderPaymentPage($payment, $invoiceId, $testMode, $texts = [])
{
    ...
    $chain            = htmlspecialchars($payment['chain'], ENT_QUOTES, 'UTF-8');
    ...
    $texts = array_merge([
        ...
        'warning'          => 'Send only ' . $tokenName . ' on ' . $chainLabel . ' to this address. Sending other tokens or using the wrong network may result in loss.',
...
function _payzcore_renderSuccess($payment, $texts = [])
{
    ...
    $explorerUrl = _payzcore_getExplorerUrl($chain, $txHash);
 ...
 *
 * @param string $chain  Chain identifier (TRC20, BEP20, ERC20, POLYGON, ARBITRUM)
 * @param string $txHash Transaction hash
 * @return string Explorer URL or empty string
 */
function _payzcore_getExplorerUrl($chain, $txHash)
{
    if (empty($txHash)) {
        return '';
    }
    ...
    $baseUrl = isset($explorers[$chain]) ? $explorers[$chain] : '';
